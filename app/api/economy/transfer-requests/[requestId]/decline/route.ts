...
import { declineTransferRequest, getProcessGhcStore } from "@/lib/server/economy/store"
import { rpcDeclineTransferRequest } from "@/lib/server/economy/db"
...
export async function POST(
  ...
  if (isDatabaseConfigured()) {
    const result = await rpcDeclineTransferRequest({
      actorId: auth.userId,
      referenceId: ref,
    })
    if (!result) {
      return jsonErr("SERVER_UNAVAILABLE", "Decline service unavailable", 503)
    }
    if (result.ok !== true) {
  ...
  const result = await declineTransferRequest(getProcessGhcStore(), {
    actorId: auth.userId,
    referenceId: ref,
  })
  if (!result.ok) {
