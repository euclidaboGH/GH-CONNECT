...
// Locked membership / boost catalog (Prompt #33)
console.log("\n19. Locked membership & boost prices")
const memDom = readFileSync(join(root, "lib/domains/membership-domain.ts"), "utf8")
assert(memDom.includes("priceGhcMonthly: 150"), "VIP monthly 150 GHC")
assert(memDom.includes("priceGhcMonthly: 300"), "VVIP monthly 300 GHC")
assert(!memDom.includes("priceGhcMonthly: 200"), "no VIP 200")
assert(!memDom.includes("priceGhcMonthly: 500"), "no VVIP 500")
const ent = readFileSync(join(root, "lib/server/membership/entitlement-store.ts"), "utf8")
assert(ent.includes("monthlyGhc: 150"), "entitlement VIP 150")
assert(ent.includes("monthlyGhc: 300"), "entitlement VVIP 300")
assert(ent.includes("monthlyPi: 1.5"), "entitlement VIP 1.5 Pi")
const spend = readFileSync(join(root, "lib/server/economy/spend-catalog.ts"), "utf8")
assert(spend.includes("BOOST") || spend.includes("boost"), "boost in catalog")
assert(spend.includes("11") || spend.includes("BOOST_TOTAL"), "boost total path")
const fulfill = readFileSync(join(root, "app/api/payments/fulfill/route.ts"), "utf8")
assert(fulfill.includes("INTENT_NOT_FOUND") || fulfill.includes("loadPaymentIntent"), "fulfill requires durable intent")
assert(!fulfill.includes("const fulfilledPayments = new Set"), "fulfill not memory-authoritative")
const activate = readFileSync(join(root, "app/api/membership/activate/route.ts"), "utf8")
assert(activate.includes("loadPaymentIntent"), "activate loads durable intent")
assert(!activate.includes("DB_SPEND_NOT_WIRED"), "activate DB spend wired")
const spendRoute = readFileSync(join(root, "app/api/economy/ledger/spend/route.ts"), "utf8")
assert(spendRoute.includes("ghc_execute_spend"), "spend RPC wired")


// P0 production safety checks
console.log("\n20. P0 production authority")
const httpTs = readFileSync(join(root, "lib/server/economy/http.ts"), "utf8")
assert(httpTs.includes("GHC_ALLOW_MEMORY_IN_PRODUCTION"), "production memory emergency flag")
assert(httpTs.includes("isProductionMemoryFinancialBlocked") || httpTs.includes("VERCEL_ENV"), "production memory guard")
const durable = readFileSync(join(root, "lib/server/economy/durable-emission.ts"), "utf8")
assert(durable.includes("ghc_activity_try_grant"), "activity durable RPC")
assert(durable.includes("ghc_record_global_demand"), "global demand RPC")
assert(durable.includes("ghc_membership_upsert"), "membership upsert RPC")
const act = readFileSync(join(root, "lib/server/economy/activity-emission.ts"), "utf8")
assert(act.includes("computeAndCommitActivityEmissionDurable"), "durable activity API")
const disc = readFileSync(join(root, "lib/discovery-mode.ts"), "utf8")
assert(disc.includes("isDiscoveryDemoAllowed"), "discovery demo gate")
const migP0 = readFileSync(join(root, "supabase/migrations/20260905_p0_activity_governor_durable.sql"), "utf8")
assert(migP0.includes("ghc_activity_try_grant"), "migration activity grant")
assert(migP0.includes("ghc_membership_upsert"), "migration membership")


// P0.1 membership DB authority + demo policy
console.log("\n21. Membership DB authority & demo isolation")
const memStore = readFileSync(join(root, "lib/server/membership/entitlement-store.ts"), "utf8")
assert(memStore.includes("getEntitlementAuthoritative"), "authoritative get")
assert(memStore.includes("loadEntitlementFromDb"), "DB load")
assert(memStore.includes("MEMBERSHIP_DURABLE_WRITE_FAILED"), "fail closed write")
assert(memStore.includes("monthlyGhc: 150"), "VIP 150 locked")
assert(memStore.includes("monthlyGhc: 300"), "VVIP 300 locked")
const statusRt = readFileSync(join(root, "app/api/membership/status/route.ts"), "utf8")
assert(statusRt.includes("getEntitlementAuthoritative"), "status uses DB authority")
const demo = readFileSync(join(root, "lib/demo-data-policy.ts"), "utf8")
assert(demo.includes("isStudioSeedEntity"), "seed entity detector")
assert(demo.includes("filterStudioSeedEntities"), "seed filter")
const grid = readFileSync(join(root, "components/ghc/discovery-grid-screen.tsx"), "utf8")
assert(grid.includes("filterStudioSeedEntities") || grid.includes("adaptDiscoveryList") || grid.includes("searchDiscoveryObjects"), "grid uses central filter")


console.log("\n22. Seed source production gate")
const ghcData = readFileSync(join(root, "lib/ghc-data.ts"), "utf8")
assert(ghcData.includes("if (!isDemoDataAllowed()) return []"), "seed functions empty in prod")
const clientSvc = readFileSync(join(root, "lib/backend/client-services.ts"), "utf8")
assert(clientSvc.includes("isDemoDataAllowed() ? seedCandidates()"), "matching no silent seed")
assert(clientSvc.includes("isDemoDataAllowed() ? seedPosts()"), "feed no silent seed")
const commScr = readFileSync(join(root, "components/ghc/communities-screen.tsx"), "utf8")
assert(commScr.includes("isDemoDataAllowed()"), "communities seed gated")
const nextCfg = readFileSync(join(root, "next.config.mjs"), "utf8")
assert(nextCfg.includes("ignoreBuildErrors: false"), "TS errors not ignored")
assert(nextCfg.includes("ignoreDuringBuilds: false"), "eslint not ignored in build")


// P0.2 — production seed isolation & fail-closed finance
console.log("\n22. P0.2 seed isolation & financial fail-closed")
const regP02 = readFileSync(join(root, "lib/domains/community-registry.ts"), "utf8")
assert(regP02.includes("isDemoDataAllowed"), "registry demo gate")
assert(regP02.includes("if (!isDemoDataAllowed()) return []"), "registry empty in prod")
assert(regP02.includes("never re-inject demo") || regP02.includes("stripDemo") || regP02.includes("demo-community-"), "registry prod no seed merge")
const commP02 = readFileSync(join(root, "components/ghc/communities-screen.tsx"), "utf8")
assert(commP02.includes("if (!isDemoDataAllowed()) return []"), "communities seed empty in prod")
const liveP02 = readFileSync(join(root, "components/ghc/discovery-components.tsx"), "utf8")
assert(liveP02.includes("isDemoDataAllowed"), "live section gated")
const evalP02 = readFileSync(join(root, "app/api/economy/rewards/evaluate/route.ts"), "utf8")
assert(evalP02.includes("SERVER_UNAVAILABLE"), "evaluate fails closed without memory/DB path")
const xferP02 = readFileSync(join(root, "app/api/economy/transfers/route.ts"), "utf8")
assert(xferP02.includes("SERVER_UNAVAILABLE"), "transfer fails closed")
const spendP02 = readFileSync(join(root, "app/api/economy/ledger/spend/route.ts"), "utf8")
assert(spendP02.includes("ghc_execute_spend"), "spend uses RPC when DB")
const memP02 = readFileSync(join(root, "lib/server/membership/entitlement-store.ts"), "utf8")
assert(memP02.includes("loadEntitlementFromDb"), "membership DB load")
assert(memP02.includes("getEntitlementAuthoritative"), "membership authoritative")
const cfgP02 = readFileSync(join(root, "lib/server/economy/economic-config.ts"), "utf8")
assert(cfgP02.includes("0.39") && cfgP02.includes("1.96"), "claim schedule")
assert(cfgP02.includes("ACTIVITY_DAILY_CAP_GHC = 0.5"), "activity day cap")
assert(cfgP02.includes("ACTIVITY_WEEKLY_CAP_GHC = 2.5"), "activity week cap")
assert(cfgP02.includes("VIP_PRICE_GHC = 150"), "VIP 150")
assert(cfgP02.includes("VVIP_PRICE_GHC = 300"), "VVIP 300")
assert(cfgP02.includes("CLAIM_USER_SHARE = 0.8"), "80% user")
